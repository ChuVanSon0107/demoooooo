...
struct Node* addPolynomial(struct Node* first, struct Node* second)
{
	...
	struct Node* tmp=NULL;//không nên dùng biến tmp để lưu các con trỏ tạm thời, nó sẽ dẫn đến trỏ linh tinh-> dẫn đến sai
	while(first != NULL && second != NULL){
		if(first->soMu == second->soMu){
			//2 số hạng có cùng số mũ
			if(first->heSo+second->heSo != 0){
				//không nên dùng biến tmp để lưu các con trỏ tạm thời, nó sẽ dẫn đến trỏ linh tinh-> dẫn đến sai
				addElementLast(&head,&last,createNewNode(first->heSo + second->heSo, first->soMu));
			}
			first=first->next;
			second=second->next;
		}
			...
			//không nên dùng biến tmp để lưu các con trỏ tạm thời, nó sẽ dẫn đến trỏ linh tinh-> dẫn đến sai
			addElementLast(&head,&last,createNewNode(first->heSo, first->soMu));
			first=first->next;
		}
		else{
			//không nên dùng biến tmp để lưu các con trỏ tạm thời, nó sẽ dẫn đến trỏ linh tinh-> dẫn đến sai
			addElementLast(&head,&last,createNewNode(second->heSo, second->soMu));
			second=second->next;			
		}
	}
	while(first != NULL){
...
struct Node* multiplyPolynomialWithNode(struct Node* head, struct Node* multiplier)
{
	...
	while(head != NULL){
		//không nên dùng biến tmp để lưu các con trỏ tạm thời, nó sẽ dẫn đến trỏ linh tinh-> dẫn đến sai
		addElementLast(&res,&last,createNewNode(head->heSo*multiplier->heSo,head->soMu+multiplier->soMu));
		head=head->next;
	}
	return res;
}
...
struct Node* multiplyTwoPolynomial(struct Node* first, struct Node* second)
{
	...
	while(second != NULL){
		//không nên dùng biến tmp để lưu các con trỏ tạm thời, nó sẽ dẫn đến trỏ linh tinh-> dẫn đến sai
		addPolynomialToCurrentPlynomial(&head,multiplyPolynomialWithNode(first,second));//cộng với head 
		second=second->next;
	}
	return head;
}
